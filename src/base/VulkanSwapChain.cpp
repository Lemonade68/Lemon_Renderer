...
namespace LR {
    ...
    void VulkanSwapChain::initSurfaceAndQueue(VkSurfaceKHR surface, GLFWwindow* window) {
        // set up swapchain surface
    ...
    * 
    * @note width and height should be decided by glfwGetFramebufferSize which retuns resolution of the surface in pixels, so need std::clamp
    */
    void VulkanSwapChain::create(uint32_t* width, uint32_t* height, bool vsync, bool fullscreen) {
        ...
        if (surfCaps.currentExtent.width == (uint32_t)-1) {
            ...
            //* using clamp because glfw returns resolution of the surface in pixels, which may lead to some questions
            extent.width  = std::clamp(*width, surfCaps.minImageExtent.width, surfCaps.maxImageExtent.width);
            extent.height = std::clamp(*height, surfCaps.minImageExtent.height, surfCaps.maxImageExtent.height);
        } else {
