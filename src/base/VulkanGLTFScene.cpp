...
// 解决重定义问题（对于header_only libraries）注意放在tinygltf之前（且所有.cpp中只需要一次）
...
*/
    ...
    // 获取当前node下的顶点及索引数据，添加到vertexBuffer以及indexBuffer
                    ...
                    switch (accessor.componentType) {
                        case TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT: {
                            ...
                            for (size_t index = 0; index < accessor.count; index++) {
                                // 为什么要加上vertexStart：buf[index]是相对于该primitive而言的，即该primitive中顶点的索引数值，但是indexBuffer中的索引是针对于整个model而言的
                                // 因此要在这个本地的顶点索引前加上之前已经有的顶点个数，才是相对于整个model而言的索引数值
                                // 举例：在该Primitive的顶点中，索引为a，但是vertexBuffer之前已经记录了别的node的共计b的顶点，因此总的索引就是b+a
