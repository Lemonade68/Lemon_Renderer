...
namespace LR {
	...
	*/
    ...
    public:
        ...
        VkSampler                              sampler;
        std::vector<LR::FramebufferAttachment> attachments;
		...
		*/
        Framebuffer(LR::VulkanDevice* vulkanDevice) {
            assert(vulkanDevice);
		...
		*/
        uint32_t addAttachment(LR::AttachmentCreateInfo createinfo) {
            LR::FramebufferAttachment attachment;
            ...
            if (createinfo.usage & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) {
                ...
                if (attachment.hasStencil()) {
                    aspectMask = aspectMask | VK_IMAGE_ASPECT_STENCIL_BIT;  // 可以既有depth也有stencil
                }
            }

            assert(aspectMask > 0);

            VkImageCreateInfo imageCI = LR::initializers::imageCreateInfo();
            imageCI.imageType         = VK_IMAGE_TYPE_2D;
            imageCI.format            = createinfo.format;
            imageCI.extent.width      = createinfo.width;
            imageCI.extent.height     = createinfo.height;
            imageCI.extent.depth      = 1;
            imageCI.mipLevels         = createinfo.miplevels;       // changed
            ...
            VK_CHECK(vkCreateImage(vulkanDevice->logicalDevice, &imageCI, nullptr, &attachment.image));
            ...
            VK_CHECK(vkAllocateMemory(vulkanDevice->logicalDevice, &memAlloc, nullptr, &attachment.memory));
            VK_CHECK(vkBindImageMemory(vulkanDevice->logicalDevice, attachment.image, attachment.memory, 0));

            attachment.subresourceRange            = {};
            attachment.subresourceRange.aspectMask = aspectMask;
            attachment.subresourceRange.levelCount = createinfo.miplevels;      // changed
            attachment.subresourceRange.layerCount = createinfo.layerCount;

            VkImageViewCreateInfo imageView       = LR::initializers::imageViewCreateInfo();
            ...
            VK_CHECK(vkCreateImageView(vulkanDevice->logicalDevice, &imageView, nullptr, &attachment.view));
